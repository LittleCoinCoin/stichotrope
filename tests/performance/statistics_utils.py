...
def calculate_overhead_statistics(
    ...
    overhead_pct = ((profiled_mean - baseline_mean) / baseline_mean * 100) if baseline_mean > 0 else 0.0
...
def check_regression(
    current_overhead_pct: float,
    baseline_overhead_pct: Optional[float],
    ...
    if delta > threshold_pct:
        return (True, f"REGRESSION: Overhead increased by {delta:.2f}% (threshold: {threshold_pct}%)")
    else:
